...
def test_format_event_exact_bytes() -> None:
    ...
    expected = 'data: {"type":"delta","data":{"content":"Hello"}}\n\n'
    assert format_event("delta", {"content": "Hello"}) == expected
...
def test_format_event_no_literal_newlines_in_json() -> None:
    ...
    data_line = payload.removeprefix("data: ").removesuffix("\n\n")
    assert "\n" not in data_line
...
def test_usage_event_embeds_five_fields() -> None:
    ...
    parsed = json.loads(evt.removeprefix("data: ").removesuffix("\n\n"))
